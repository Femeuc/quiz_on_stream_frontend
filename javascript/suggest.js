...
function send_question() {
    ...
    if(descripion.length < 1 || option_a.length < 1 || option_b.length < 1 || option_c.length < 1 || option_d.length < 1
        ...
        alert("Sua sugestão não foi enviada porque você deixou algum campo vazio ou escreveu algo inválido.");
        return;
    }

    //http://localhost:3000/question
    //https://quiz-on-stream.herokuapp.com/question
    fetch("https://quiz-on-stream.herokuapp.com/question/suggest", {
    ...
    .then(async function (response) {
        if(response.status == 200) {
            alert("Questão sugerida com sucesso!");
            clear_fields();
            ...
            alert("Erro ao sugerir. Status: " + response.status);
    })
}
...
function send_subject(is_general_subject) {
    ...
    fetch("https://quiz-on-stream.herokuapp.com/questions/subject/suggest", {
    ...
    .then(function (response) {
        if(response.status == 200) {
            alert("Assunto sugerido com sucesso!");
            subject_to_add.value = "";
        }
        else
            alert("Erro ao enviar sugestão. Status: " + response.status);
    })
}

function add_question_explanation(text, question_id) {
    fetch("https://quiz-on-stream.herokuapp.com/question/explanation/suggestion", {
        method: "POST",
        body: JSON.stringify({
            text: text,
